...
def resize_image(input_image, resolution):
    if isinstance(resolution, int):
        ...
        img = cv2.resize(input_image, (resolution[0], resolution[1]))
    return img


def prepare_frames(input_path: str, output_dir: str, resolution, crop, use_limit_device_resolution=False):
    l, r, t, b = crop
...
def resize_image(input_image, resolution):
    img = cv2.resize(input_image, resolution)
    return img

def prepare_frames_lavie(input_path: str, output_dir: str, resolution, crop, use_limit_device_resolution=False):
    l, r, t, b = crop

    def crop_func_lavie(frame):
        H, W, C = frame.shape
        left = np.clip(l, 0, W)
        right = np.clip(W - r, left, W)
        top = np.clip(t, 0, H)
    ...
    video_to_frame(input_path, output_dir, '%04d.png', False, crop_func_lavie)
